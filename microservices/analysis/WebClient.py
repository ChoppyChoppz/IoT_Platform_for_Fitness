...
class WebClient(object):
    ...
    def post_weight(self, msg):
        cat_url = "http://" + self.config["cat_ip"] + ":" + self.config["cat_port"] + "/new_weight"
        print("Sending a POST request to %s " %(cat_url))
...
def main():
    ...
    """ prova=json.dumps(WB.get_patients())
    print(prova)


    lista_device=json.dumps(WB.get_devices('p_1001'))
    name_device=list(lista_device.values())
    print(name_device) """

    """ info_patient=json.dumps(WB.get_info_patient('p_1001'))
    print(json.loads(info_patient)) """

    """ broker=json.dumps(WB.get_broker())
    print(broker) """

    """ ts=json.dumps(WB.get_ts())
    print(ts) """

    """ info_device=json.dumps(WB.get_info_devices('p_1001'))
    print(json.loads(info_device)) """

    """ info_TS_delete=json.dumps(WB.get_patients_TS())
    print(json.loads(info_TS_delete)) """
